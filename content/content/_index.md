...
<script>
  (function() {
    console.log("[Omni-Pulse] Script loaded in _index.md");

    function initOmniPulse() {
      console.log("[Omni-Pulse] initOmniPulse running...");
      ...
      // Prevent duplicate tickers if the script runs twice
      if (document.getElementById("omni-pulse-wrapper")) return;
    ...
    // Fire immediately if page is already loaded, otherwise wait for DOM
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", initOmniPulse);
    } else {
      initOmniPulse();
    }
  })();
